...
import AccommodationCreateForm from './components/AccommodationCreateForm';
...
const Accommodation: FC = () => {
	...
	const handleExcelDownload = async () => {
		...
		try {
			const res = await getItemList({ page: 1, countPerPage: 999, type: '숙박', enable: true });
	...
	return (
		<S.Container>
			<Title title="Accommodation Management" desc="You can manage accommodation.">
				<ButtonView>
					<Button type="button" status="third_outlined" text="Excel Download" onClick={handleExcelDownload} />
					<Button
						type="button"
						status="third"
						text={needCreateForm ? 'Collapse' : 'Add to accommodation'}
						onClick={() => {
							if (needCreateForm) {
								setMode('select');
							} else {
			...
			<Free.SearchTable onSubmit={handleSubmit(onSubmit)}>
				...
				<Free.Row>
					<Free.Value $width={50}>
						<TextField
							label="Accommodation Name(Korean)"
							name="nameKor"
							placeholder="Please enter the accommodation name in Korean."
							register={register}
							// errors={errors}
					...
					<Free.Value>
						<TextField
							label="Accommodation Name(English)"
							name="nameEng"
							placeholder="Please enter the accommodation name in English."
							register={register}
							// errors={errors}
			...
			{/*  */}
			{needCreateForm && (
				<AccommodationCreateForm needCreateForm={needCreateForm} editTarget={editTarget} refetch={refetchItemList} />
			)}
			...
			<Loader isFetching={isLoading || isLoadingDelete || isLoadingCopy || isLoading} />
