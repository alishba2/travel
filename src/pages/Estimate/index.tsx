...
const Estimate: FC = () => {
	...
	const goDetail = (id: string) => {
		navigate(PATHS.ESTIMATE_DETAIL.replace(':id', id));
	};
	...
	return (
		<S.Container>
			<Title title="견적 리스트 조회" desc="견적 리스트를 조회할 수 있습니다.">
				{/* <Button
					type="submit"
					status="third"
			...
			<Free.ResultTable>
				...
				<Free.Row>
					<Free.Value $width={6}>1</Free.Value>
					<Free.Value $width={10}>경복궁</Free.Value>
					<Free.Value $width={10}>하성민</Free.Value>
					<Free.Value $width={8}>1박2일</Free.Value>
					<Free.Value $width={15}>{comma(1000000)}원</Free.Value>
					<Free.Value $width={15}>{getYmd('20230101', 'YYYY-MM-DD HH:mm')}</Free.Value>
					<Free.Value $width={8}>1</Free.Value>
					<Free.Value $width={8}>답변 전</Free.Value>
					<Free.Value>
						<OptionButtons>
