...
import { comma, getItemImg, unit } from '@shared/utils/base';
...
const Place: FC = () => {
	...
	const handleExcelDownload = async () => {
		...
		try {
			const res = await getItemList({ page: 1, countPerPage: 999, type: '여행지', enable: true });
	...
	return (
		<S.Container>
			<Title title="Place Management" desc="You can manage places.">
				<ButtonView>
					<Button type="button" status="third_outlined" text="Excel Download" onClick={handleExcelDownload} />
					<Button
						type="button"
						status="third"
						text={needCreateForm ? 'Collapse' : 'Add Place'}
						onClick={() => {
							if (needCreateForm) {
								setMode('select');
							} else {
			...
			{/*  */}
			{needCreateForm && (
				<PlaceCreateForm needCreateForm={needCreateForm} editTarget={editTarget} refetch={refetchItemList} />
			)}
					...
					targetList.map((o, index) => {
						...
						return (
							<Free.Row key={id} $isLast={isLast}>
								...
								<Free.Value $width={15} style={{ display: 'flex', flexDirection: 'column', gap: unit(5) }}>
									<div>{nameEng}</div>
									<div>{nameKor}</div>
								</Free.Value>
