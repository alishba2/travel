...
import TransportationCreateForm from './components/TransportationCreateForm';
...
const Transportation: FC = () => {
	...
	const handleExcelDownload = async () => {
		...
		try {
			const res = await getItemList({ page: 1, countPerPage: 999, type: '이동수단', enable: true });
	...
	return (
		<S.Container>
			<Title title="Transportation Management" desc="You can manage transportation.">
				<ButtonView>
					<Button type="button" status="third_outlined" text="Excel Download" onClick={handleExcelDownload} />
					<Button
						type="button"
						status="third"
						text={needCreateForm ? 'Collapse' : 'Add to transportation'}
						onClick={() => {
							if (needCreateForm) {
								setMode('select');
							} else {
			...
			<Free.SearchTable onSubmit={handleSubmit(onSubmit)}>
				...
				<Free.Row>
					<Free.Value $width={50}>
						<TextField
							label="Transportation Name(Korean)"
							name="nameKor"
							placeholder="Please enter the Transportation Name in Korean."
							register={register}
							// errors={errors}
					...
					<Free.Value>
						<TextField
							label="Transportation Name(English)"
							name="nameEng"
							placeholder="Please enter the Transportation ame in English."
							register={register}
							// errors={errors}
			...
			{/*  */}
			{needCreateForm && (
				<TransportationCreateForm needCreateForm={needCreateForm} editTarget={editTarget} refetch={refetchItemList} />
			)}
			...
			<Free.ResultTable>
				<Free.Row>
					...
					<Free.Label $width={10}>Price (USD)</Free.Label>
					<Free.Label $width={20}>Description</Free.Label>
				...
				{!isUndefined(targetList) &&
					targetList.map((o, index) => {
						const { id, enable, nameEng, nameKor, price, description, files, grade } = o;
						...
						return (
							<Free.Row key={id} $isLast={isLast}>
								...
								<Free.Value $width={10}>{comma(price)}</Free.Value>
								<Free.Value $width={20}>{description}</Free.Value>
			...
			<Loader isFetching={isLoading || isLoadingDelete || isLoadingCopy || isLoadingExcel} />
		</S.Container>
	);
};
