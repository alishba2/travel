...
const TransportationCreateForm: FC<Props> = ({ needCreateForm, editTarget, refetch }) => {
	/**
	 * States
	 */
	...
	useEffect(() => {
		...
		const { description, enable, files, id, nameEng, nameKor, price, pricePolicy, keyword, websiteLink, personalTag } =
			editTarget;
	...
	const onSubmit: SubmitHandler<IFormValues> = async (data) => {
		console.log(data);
		const { description, enable, nameEng, nameKor, price, thumbnail, keyword, websiteLink, personalTag } = data;

		const hasEmptyValue = priceArray.some((value) => value === '');
		if (priceArray.length !== 1 && hasEmptyValue) {
			errorToast('Please enter the price.');
			return;
		}
	...
	return (
		<>
			<Blank size={20} />
			<Title title="Add/Edit Transportation" desc="You can add or edit transportation.">
				...
				<Free.Row>
					...
					<Free.Value>
						<TextField
							...
							name="nameEng"
							placeholder="Please enter the transportation name in English."
							register={register}
							errors={errors}
...
export default TransportationCreateForm;
